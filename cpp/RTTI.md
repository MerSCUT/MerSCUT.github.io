...
`dynamic_cast` 的最有价值的应用是, __在包含虚函数的类体系中, 让基类指针/引用安全地转换为派生类指针__. 

- __向上转换__ : 派生类转为基类指针/引用, 此时**不需要检查类型.**
...
3. 补 : 在__多重继承中__, `type_info` 还会指导`dynamic_cast` 计算出(this)指针偏移量.
...
该方法运行在__编译期__, 且不要求有 RTTI 机制, 也就不要求类有虚函数.
